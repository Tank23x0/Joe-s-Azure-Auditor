...
If SharePoint Online is disconnected you will be prompted for the admin URL. To skip SharePoint/OneDrive sections explicitly:
```powershell
pwsh ./scripts/Run-FullAudit.ps1 -OutputFormat Html -OutputPath ./reports -SkipSharePointIfDisconnected
```
...
- To reset sessions between runs, use `Disconnect-MgGraph`, `Disconnect-ExchangeOnline`, and `Disconnect-SPOService`.
