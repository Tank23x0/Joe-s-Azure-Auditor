...
function Initialize-AuditorConnections {
    ...
    if ($loadedModules -contains 'Microsoft.Online.SharePoint.PowerShell') {
        Write-AuditorMessage -Message 'Connect-SPOService required for SharePoint tenant settings (use https://<tenant>-admin.sharepoint.com).' -Level 'WARN'
    }
...
foreach ($sectionName in $requestedSections) {
    switch ($sectionName) {
        ...
        'SharePoint' { $report.Sections += Get-SharePointSection }
        'OneDrive' { $report.Sections += Get-OneDriveSection }
